...
export default function Login() {
	...
	const Auth = async (e) => {
		...
		if (username.length !== 0 && password.length !== 0) {
			try {
				let res = await axios.post("http://localhost:5106/api/login", {
					userName: username,
					password: password,
				});
		...
		<>
			<div className="registerBody">
				<form className="form-signup center-middle" onSubmit={Auth}>
					...
					<div className="row">
						<div class="col-md-6">
							<input className="buttonLog" type="submit" class="buttonLog btn-success btn-block submit-button" name="" value="Submit"></input>
						</div>
						<div class="col-md-6">
					...
					<p>
						...
						<Button className="buttonLog" variant="link" onClick={directToRegister}>
							Sign up
						</Button>
					</p>
				</form>
